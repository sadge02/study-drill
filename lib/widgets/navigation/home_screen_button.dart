import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:study_drill/utils/constants/core/general_constants.dart';
import 'package:study_drill/utils/constants/navigation/widgets/home_screen_button_widget_constants.dart';

/// A customizable button widget for the home screen dashboard.
///
/// [HomeScreenButton] displays a button with an icon and label text.
/// The button features a white background with a primary color border and shadow.
class HomeScreenButton extends StatelessWidget {
  const HomeScreenButton({
    super.key,
    required this.label,
    required this.icon,
    required this.onTap,
  });

  /// The text label displayed inside the button.
  final String label;

  /// The icon displayed to the left of the label.
  final IconData icon;

  /// Callback triggered when the button is tapped.
  final VoidCallback onTap;

  @override
  Widget build(BuildContext context) {
    debugPrint('HomeScreenButton: Building button - $label');
    return Container(
      height: HomeScreenButtonWidgetConstants.buttonHeight,
      decoration: _buildDecoration(),
      child: Material(
        color: Colors.transparent,
        child: InkWell(
          borderRadius: BorderRadius.circular(
            GeneralConstants.mediumCircularRadius,
          ),
          onTap: () {
            debugPrint('HomeScreenButton: Tapped - $label');
            onTap();
          },
          child: Padding(
            padding: const EdgeInsets.symmetric(
              horizontal: GeneralConstants.smallPadding,
            ),
            child: _buildContent(),
          ),
        ),
      ),
    );
  }

  /// Builds the decoration (background, border, border-radius, and shadow) for the button.
  BoxDecoration _buildDecoration() {
    return BoxDecoration(
      color: Colors.white,
      borderRadius: BorderRadius.circular(
        GeneralConstants.mediumCircularRadius,
      ),
      border: Border.all(
        color: GeneralConstants.primaryColor.withValues(
          alpha: HomeScreenButtonWidgetConstants.buttonOpacity,
        ),
      ),
      boxShadow: [
        BoxShadow(
          color: Colors.black.withValues(
            alpha: HomeScreenButtonWidgetConstants.buttonShadowOpacity,
          ),
          blurRadius: HomeScreenButtonWidgetConstants.buttonBlurRadius,
          offset: const Offset(
            HomeScreenButtonWidgetConstants.buttonXOffset,
            HomeScreenButtonWidgetConstants.buttonYOffset,
          ),
        ),
      ],
    );
  }

  /// Builds the content (icon and label) displayed inside the button.
  Widget _buildContent() {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        _buildIcon(),
        const SizedBox(width: GeneralConstants.tinySpacing),
        _buildLabel(),
      ],
    );
  }

  /// Builds the icon widget displayed on the left side of the button.
  Widget _buildIcon() {
    return Icon(
      icon,
      color: GeneralConstants.primaryColor,
      size: GeneralConstants.smallIconSize,
    );
  }

  /// Builds the label text widget with ellipsis overflow handling.
  Widget _buildLabel() {
    return Flexible(
      child: Text(
        label,
        overflow: TextOverflow.ellipsis,
        maxLines: HomeScreenButtonWidgetConstants.labelMaxLines,
        style: GoogleFonts.lexend(
          fontSize: GeneralConstants.smallFontSize,
          fontWeight: FontWeight.w500,
          color: GeneralConstants.primaryColor,
        ),
      ),
    );
  }
}
